"""Base test class for Tree-of-Thought search strategy tests."""
...
dspy.configure(
    lm=dspy.LM(
        model=os.getenv("MODEL_ID", "openrouter/meta-llama/llama-3.1-8b-instruct"),
...
class BaseToTTest:
    """Base class for Tree-of-Thought search strategy tests."""
    
    STRATEGY = None  # To be overridden by subclasses
    
    def generate_fn(self, question):
        """Generate function for creating new states."""
    ...
    def evaluate_fn(self, states):
        """Evaluate function for scoring states."""
        scores = []
        for s in states:
    ...
    def is_goal_fn(self, state):
        """Goal function to determine if a state meets completion criteria."""
        text = state.lower()
        words = text.split()
        has_dspy = 'dspy' in words
        long_enough = len(words) > 120
        key_terms = {'signatures', 'compilation', 'teleprompters', 'modules'}
        coverage = sum(1 for t in key_terms if t in text) >= 2
        has_structure = ('###' in state) or ('- ' in state)
        return has_dspy and long_enough and coverage and has_structure

    @pytest.mark.skipif(not os.getenv('OPENROUTER_API_KEY'), reason='OPENROUTER_API_KEY not set')
    @pytest.mark.parametrize('query', load_queries(get_profile()))
    def test_run_queries(self, query):
        """Run test queries with the specific search strategy."""
        if self.STRATEGY is None:
            pytest.skip("STRATEGY not defined in subclass")
        ...
        if ind_scores:
            m = sum(ind_scores) / len(ind_scores)
            score_variance = sum((x - m) ** 2 for x in ind_scores) / len(ind_scores)
        else:
            score_variance = 0.0
